...
def configurar_administrador_principal(*, username, password=None, substituir=False):
    ...
    if not username:
        raise ValidationError({"username": "Informe um nome de usuário válido."})
    ...
    if havia_admins_conflitantes and not substituir:
        raise ValidationError(
            {
                "detail": (
                    "Já existe um administrador principal configurado. "
                    "Use o comando com --substituir para transferir a administração."
                )
            }
        )
    ...
    if password:
        ...
        raise ValidationError(
            {"password": "Informe uma senha válida para o administrador principal."}
        )
